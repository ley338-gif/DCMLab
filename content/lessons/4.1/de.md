...
**Was du daran abliest:** Der TCP-Verbindungsversuch wurde aktiv zurückgewiesen (`ECONNREFUSED`) — häufig durch einen RST wegen eines fehlenden Listeners auf dem angesprochenen Port, möglich ist aber auch eine aktive Ablehnung durch eine Firewall oder Netzwerkkomponente auf dem Weg dorthin, nicht zwingend vom Zielhost selbst. Sicher ist nur die aktive Zurückweisung des Verbindungsversuchs — nicht, welches Gerät genau sie ausgesprochen hat. „Connection refused" beweist deshalb nicht pauschal, dass der Zielhost geantwortet hat oder überhaupt sicher erreichbar ist, und auch nicht zwingend „hier läuft überhaupt kein Dienst". Ein reiner Timeout (gar keine Antwort statt einer sofortigen Ablehnung) sieht dagegen komplett anders aus und deutet eher auf ein still verwerfendes `DROP` oder eine Route ins Leere — ein eigenes Fehlerbild mit eigener Ursache, siehe Lektion 4.4.
...
**Was du daran abliest:** Eine andere Fehlermeldung für ein anderes Problem. Allgemein bedeutet `ENETUNREACH`: Der eigene Netzwerkstack meldet das Zielnetz als nicht erreichbar — mehr sagt der Fehlercode für sich genommen nicht aus. Für den hier konkret nachgebauten, isolierten Docker-Aufbau lässt sich das genauer fassen: Das Egress-lose interne Docker-Netz hatte in diesem Testaufbau schlicht keine Route nach außen, die Anfrage verließ den lokalen Host deshalb gar nicht erst in Richtung des Ziels — dieselbe Absicherung, die die echte Spielwiese laut ADR 0008 bewusst einsetzt. Vier Meldungen für vier unterscheidbare Ursachen, die sich nicht gegenseitig ersetzen: `Connection refused` (der Verbindungsversuch wurde irgendwo aktiv zurückgewiesen — Zielhost oder Firewall/Netzwerkkomponente, nicht zwingend „kein Dienst"), `Network unreachable` (der eigene Netzwerkstack meldet das Zielnetz allgemein als nicht erreichbar), `Host unreachable` (typischerweise: eine Route zum Zielnetz existiert, aber der konkrete Zielhost antwortet auf dieser Route nicht — hier nicht live geprüft) und ein reiner Timeout (gar keine Antwort). Die ersten beiden lassen sich hier live zeigen; `Host unreachable` und der reine Timeout brauchen eine andere Netzwerktopologie und sind Thema von Lektion 4.4.
...
| `echoscu` liefert `Connection refused` | Der TCP-Verbindungsversuch wurde aktiv zurückgewiesen (`ECONNREFUSED`, häufig per RST) — durch den Zielhost selbst oder ein Gerät auf dem Weg dorthin | dass der Zielhost selbst geantwortet hat, dass generell kein Dienst existiert, oder dass der Host sicher erreichbar ist | Port und Erreichbarkeit mit dem Betreiber der Gegenstelle abgleichen |
...
3. Zum Zeitpunkt dieses Gegenchecks war der adressierte DICOM-Dienst nicht vollständig ausgefallen oder unerreichbar
