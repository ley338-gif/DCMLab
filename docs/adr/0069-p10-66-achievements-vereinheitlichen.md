# 0069 — P10.66: Achievements vereinheitlichen
...
$user): Collection`.** Sie führt beide Quellen zu einer nach `awarded_at`
...
- `Dashboard.vue`: `AchievementEntry`-Typ und `achievementLabels` erweitert
  um `track_passed`; neuer lang-Key `"Abschlussprüfung bestanden"`.
...
- `DashboardTest`: erweitert um den Nachweis, dass ein `TrackBadge` in der
  Achievements-Liste des Dashboards auftaucht.
