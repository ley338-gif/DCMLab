...
from app.db import ScenarioSession, get_db, init_db
...
app = FastAPI(
    title="DCM Lab Scenario Engine",
    description="Entscheidungsbaum-Engine fuer nicht-technische Themenfelder "
    "(Abschnitt 13). Kennt Nodes und Sitzungen, keine Nutzer -- wie services/engine.",
    lifespan=lifespan,
)
...
def _get_session_or_404(session_id: str, db: Session) -> ScenarioSession:
    row = db.get(ScenarioSession, session_id)
...
def _save(db: Session, row: ScenarioSession) -> None:
...
def create_session(body: CreateSessionRequest, db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    row = ScenarioSession(node_slug=node.slug, state=rules.initial_state(node))
    db.add(row)
    db.commit()
    db.refresh(row)
